...
export function GetCarsType() {
  return axios.get('http://localhost:83/api/Cars').then(x => {
    const data = { ...x.data }
    const option = []
    for (let x in data) {
...
export function UpdateDurationOrder(id) {
  ...
  return axios.put(`http://localhost:83/api/PutUpdateDurationOrder?DriverId=${sessionStorage.getItem("userId")}`).then(x => {
...
export function AllOrders() {
  return axios.get(`http://localhost:83/api/order?Id=${sessionStorage.getItem('userId')}`).then(x => {
    const data = { ...x.data }
    let neworders = []
    let orders = []
    for (let x in data) {
...
export const driversrating = (Ord_Kod, rating) => {
  return axios.put(`http://localhost:83/api/UpdateRating?IdOrder=${Ord_Kod}&num=${rating}`).then(x => {
    return
  }).catch(err => {
    if (err.message === "Network Error") {
      throw new Error( "בעיה במערכת, נסה מועד מאוחר יותר");
...
export function DriverOrder(Ord_Id_driver) {
  return axios.get(`http://localhost:83/api/driver?Id=${Ord_Id_driver}`).then(x => {
    const data = { ...x.data }
    return data;
  })
}
...
export function CustomerOrder(Cust_send) {
  return axios.get(`http://localhost:83/api/customer?Id=${Cust_send}`).then(x => {
    const data = { ...x.data }
    return data;
  })
}
...
export const StatusOrder = (order) => {
  return axios.put('http://localhost:83/api/order', order).then(x => {
    const data = { ...x.data }
    return data;
  }).catch(err => {
    if (err.message === "Network Error") {
...
export function AllOrdersByDrivers() {
  return axios.get(`http://localhost:83/api/DriverToOrder?IdDriver=${sessionStorage.getItem("userId")}`).then(x => {
    const data = { ...x.data }
    let neworders = []
    let orders = []
    for (let x in data) {
