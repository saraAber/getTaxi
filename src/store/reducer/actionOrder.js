...
export const orderfunc = (order) => {
    ...
    return dispatch => {
        ...
        axios.post('http://localhost:83/api/PostOrder', order)
            .then(x => {
                const Order = { ...x.data }
                dispatch({ type: updateorder, Neworder: Order })
                axios.post('http://localhost:83/api/PostDrivers', Order).then(x => {
...
export const calculateprice = (Order) => {
    return dispatch => {
        axios.post('http://localhost:83/api/Calculatingprice', Order).then(x => {
...
export const OrderConfirmed = (Ord_Kod) => {
    return dispatch => {
      axios.get(`http://localhost:83/api/IsOrderConfirmed?KodOrder=${Ord_Kod}`).then(x => {
        const order = { ...x.data }
